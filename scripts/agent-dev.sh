...
# "deepseek-balance-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "deepseek-balance-dev" is overwritten by
...
PROJECT_NAME="deepseek-balance"
AGENT_NAME="deepseek-balance-dev"
WORKSPACE="/workspaces/deepseek-balance"
...
if [ -x "/workspaces/deepseek-balance/scripts/agent-dev.sh" ]; then
  "/workspaces/deepseek-balance/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
